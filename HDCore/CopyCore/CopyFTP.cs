...
namespace CopyFile
{
    public class FTP : ICopy
    {
        ...
        public void OpenUpload(string filename, string remote_filename, bool resume)
        {
            ...
            this.fileDestName = remote_filename;
            return;
        }
        /// <summary>
        ...
        public void OpenDownload(string remote_filename, string local_filename, bool resume)
        {
            ...
            this.fileSourceName = remote_filename;
            this.fileDestName = local_filename;
            return;
        }
        /// <summary>
        ...
        {
            Byte[] bytes = new Byte[512 * 1024]; // 512K
            long bytes_got;
        ...
        public void OpenDownLoadBuffer(string fileName)
        {
            Connect();
            SetBinaryMode(true);
        ...
        public long DoDownLoadBuffer(out Buffer buffer)
        {
            buffer = new Buffer();
            try
            {
                buffer.bytes_got = data_sock.Receive(buffer.buffer, buffer.buffer.Length, 0);
        ...
        public void OpenUpLoadBuffer(string fileName)
        {
            Connect();
            SetBinaryMode(true);
            OpenDataSocket();
        ...
        public long DoUpLoadBuffer(Buffer buffer)
        {
            try
            {
                bytes_total += buffer.bytes_got;
                data_sock.Send(buffer.buffer, (int)buffer.bytes_got, 0);
        ...
        public void CancelDownloadBuffer()
        {
            try
            {
                CloseDataSocket();
        ...
        public void CancelUploadBuffer()
        {
            try
            {
                // the upload is complete or an error occured
        ...
        {
            return DoUpLoadBuffer(buff);
        }

        public void TryGetFileSourceSize()
        {
            this.file_size = GetFileSize(fileSourceName);
        }

        public void TryGetFileDestSize()
        {
            this.file_size = GetFileSize(fileDestName);
        }

        public void RemoveDestFile()
        {
            RemoveFile(fileDestName);
        }
    }
}
